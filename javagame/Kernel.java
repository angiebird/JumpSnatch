			...
			if(dist == 2){
				...
				for(int i = 0; i < width; i++){
					for(int j = 0; j < height; j++){
						...
						if(jumpable(p, pj) == 1){
							msg = "It is still able to jump from "+"x:" + p.x + " y:"+p.y + " to " + "x:" + pj.x + " y:"+pj.y;
							return 0;
						}
					}
				}
				...
				if(dist_to_ctr(start) > dist_to_ctr(end)){
					...
					for(int i = 0; i < width; i++){
						for(int j = 0; j < height; j++){
							...
							Coordinate pj = new Coordinate();
							if(movable(p, pj, 2) == 1){
								msg = "You still can move to the center. Hint: "+"x:" + p.x + " y:"+p.y + " to " + "x:" + pj.x + " y:"+pj.y;
								return 0;
							}
						}
					}
					...
					if(dist_to_ctr(start) < dist_to_ctr(end)){
						...
						for(int i = 0; i < width; i++){
							for(int j = 0; j < height; j++){
								...
								Coordinate pj = new Coordinate();
								if(movable(p, pj, 1) == 1){
									msg = "You have choice to NOT move away from center. Hint: " + "x:" + p.x + " y:"+p.y + " to " + "x:" + pj.x + " y:"+pj.y;
									return 0;
								}
							}
						}
